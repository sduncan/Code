...
====================*/
struct matrix * make_rotX(double degree) {
...
====================*/
struct matrix * make_rotY(double degree) {
  double theta = (degree * M_PI) / 180;
  struct matrix * roty = new_matrix(4, 4);
  ident(roty);
  roty->m[0][0] = cos(theta);
  roty->m[0][3] = -sin(theta);
...
====================*/
struct matrix * make_rotZ(double degree) {
  double theta = (degree * M_PI) / 180;
...
/*======== struct matrix * generate_curve_coefs() ==========
  ...
  jdyrlandweaver
  ====================*/
struct matrix * generate_curve_coeffs( double p1, double p2, 
				      double p3, double p4, int type) {
  struct matrix * bh = new_matrix(4, 4);
